...
class Market
  ...
  def self.cheapest_producer(type)
    producers = $producers.find_all {|f| f.supply[type] > 0}
    producers.min_by{|f| f.price[type]}
  end
end
...
# Make some producers and consumers
$producers = []
...
SIMULATION_DURATION.times do |t|
  ...
  # the supply here is the starting supply generated in $producers
  # ... and it's the market's whole supply, not one producer's
  ...
  until Market.demand == 0 or Market.supply(cheaper_type) == 0 do
    $consumers.each do |consumer|
...
write("price_data_control", price_data)
write("supply_data_control", supply_data)